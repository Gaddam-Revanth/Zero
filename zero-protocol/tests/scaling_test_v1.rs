...
async fn multi_node_scaling_simulation_v1() {
    ...
    for name in names {
        let n = ZeroNode::new(
            tmp_dir.join(name).to_str().unwrap().to_string(),
            format!("pass_{}", name),
        )
        .unwrap();
        nodes.push(n);
    ...
    let onion_packet = {
        ...
        for node in &nodes[1..4] {
            let shared =
                ephemeral.diffie_hellman(&zero_crypto::dh::X25519PublicKey(node.self_id.idk_pub()));
        ...
        zero_dht::onion::OnionPacket::wrap_3_hops(
            b"TARGET_REACHED",
            &hops,
