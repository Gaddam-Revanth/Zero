...
use zero_crypto::{
    ...
    kdf::{hkdf, hkdf_expand, hkdf_extract, KdfContext},
};
...
impl NoiseHandshakeState {
    ...
    pub fn write_message1(&mut self) -> Result<Vec<u8>, HandshakeError> {
        if self.role != NoiseRole::Initiator || self.step != 0 {
            return Err(HandshakeError::NoiseError(
                "Invalid state for write_message1".into(),
            ));
        }
    ...
    pub fn read_message1_write_message2(&mut self, msg1: &[u8]) -> Result<Vec<u8>, HandshakeError> {
        if self.role != NoiseRole::Responder || self.step != 0 {
            return Err(HandshakeError::NoiseError(
                "Invalid state for read_message1_write_message2".into(),
            ));
        }
        if msg1.len() < 32 {
        ...
        let s_ct = self
            .sym
            .encrypt_and_hash(&self.local_static.public_key().0)?;
        // es
    ...
    pub fn read_message2_write_message3(
        ...
        if self.role != NoiseRole::Initiator || self.step != 1 {
            return Err(HandshakeError::NoiseError(
                "Invalid state for read_message2_write_message3".into(),
            ));
        }
        let mut pos = 0;
        ...
        if msg2.len() < pos + rs_ct_len {
            return Err(HandshakeError::MessageLength {
                expected: pos + rs_ct_len,
                got: msg2.len(),
            });
        }
    ...
    pub fn read_message3(&mut self, msg3: &[u8]) -> Result<Vec<u8>, HandshakeError> {
        if self.role != NoiseRole::Responder || self.step != 1 {
            return Err(HandshakeError::NoiseError(
                "Invalid state for read_message3".into(),
            ));
        }
        if self.remote_ephemeral.is_none() {
            return Err(HandshakeError::NoiseError(
                "Missing remote ephemeral".into(),
            ));
        }

        // s (remote static, encrypted)
        let rs_ct_len = 32 + 16;
        if msg3.len() < rs_ct_len {
            return Err(HandshakeError::MessageLength {
                expected: rs_ct_len,
                got: msg3.len(),
            });
...
mod tests {
    ...
    fn test_noise_xx_full_handshake() {
        ...
        let msg3 = alice
            .read_message2_write_message3(&msg2, b"ZKX payload")
            .unwrap();
