...
mod handshake_tests {
    ...
    fn test_zkx_with_opk_master_secrets_match() {
        ...
        assert_eq!(
            alice_ms.0, bob_ms.0,
            "Master secrets must match when OPK is used"
        );
    }
    ...
    fn test_zkx_without_opk_master_secrets_match() {
        ...
        assert_eq!(
            alice_ms.0, bob_ms.0,
            "Master secrets must match without OPK"
        );
    }
    ...
    fn test_zkx_replay_attack_rejected_without_opk() {
        ...
        let alice_id = ZeroId::from_keypair(&alice_kp, [0u8; 4]);
        let (init_msg, _alice_ms) = initiator
            .initiate(&alice_id, &alice_kp, &bob_bundle)
            .unwrap();
        ...
        assert!(
            result.is_err(),
            "ZKX replay (same EK, no OPK) must be rejected"
        );
    }
    ...
    fn test_zkx_different_alice_produces_different_master_secret() {
        ...
        let (init1, ms1) = X3dhInitiator::new(X25519Keypair::generate())
            .initiate(&alice_id1, &alice1, &bundle1)
            .unwrap();
        let (init2, ms2) = X3dhInitiator::new(X25519Keypair::generate())
            .initiate(&alice_id2, &alice2, &bundle2)
            .unwrap();
        ...
        assert_ne!(
            ms1.0, ms2.0,
            "Different sessions must have different master secrets"
        );
    }
    ...
    fn test_zkx_wrong_kem_ciphertext_produces_wrong_secret() {
        ...
        let initiator = X3dhInitiator::new(X25519Keypair::generate());
        let alice_id = ZeroId::from_keypair(&alice_kp, [0u8; 4]);
        let (mut init_msg, alice_ms) = initiator
            .initiate(&alice_id, &alice_kp, &bob_bundle)
            .unwrap();
        ...
        if let Ok((bob_ms, _)) = X3dhResponder::respond(&mut bob1, &init_msg) {
            assert_ne!(
                alice_ms.0, bob_ms.0,
                "Corrupted KEM ciphertext must produce different shared secret"
            );
        }
    ...
    fn test_noise_xx_full_handshake_and_message_exchange() {
        ...
        use zero_handshake::noise::{HandshakePrologue, NoiseHandshakeState, NoiseRole};
        ...
        let msg3 = alice
            .read_message2_write_message3(&msg2, b"initial payload")
            .unwrap();
        ...
        let _ct = encrypt(
            &alice_out.send_key,
            &zero_crypto::aead::AeadNonce::random(),
            pt,
            b"",
        )
        .unwrap();
    ...
    fn test_noise_xx_spk_rotation_old_spk_accepted() {
        ...
        let initiator = X3dhInitiator::new(X25519Keypair::generate());
        let alice_id = ZeroId::from_keypair(&alice_kp, [0u8; 4]);
        let (init_msg, _) = initiator
            .initiate(&alice_id, &alice_kp, &bob_bundle_before_rotate)
            .unwrap();
        ...
        assert!(
            result.is_ok(),
            "Old SPK index must be accepted from old_spks history"
        );
    }
}
