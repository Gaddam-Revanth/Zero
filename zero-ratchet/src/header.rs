...
impl DecryptedHeader {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Option<Self> {
        ...
        Some(Self { dh_pub, prev_counter, counter })
    }
}
...
pub fn decrypt_header(hk: &AeadKey, enc: &EncryptedHeader) -> Result<DecryptedHeader, RatchetError> {
    let blob = &enc.0;
    if blob.len() < 12 { return Err(RatchetError::HeaderDecryptionFailed); }
